...
package feathers.skins
{
	 ...
	 * attempts to reuse the existing starling.display.Image instance that is
	 * passed in to getValueForState() as the old value by swapping the
	 * textures and scale9Grid.
	 */
	public class Scale9ImageStateValueSelector extends StateWithToggleValueSelector
	{
		...
		override public function updateValue(target:Object, state:Object, oldValue:Object = null):Object
		{
			...
			if(oldValue is Image)
			{
				...
				image.texture = textures.texture;
				image.scale9Grid = textures.scale9Grid;
				image.readjustSize();
			}
			else
			{
				image = new Image(textures.texture);
				image.scale9Grid = textures.scale9Grid;
				image.tileGrid = null;
			}
